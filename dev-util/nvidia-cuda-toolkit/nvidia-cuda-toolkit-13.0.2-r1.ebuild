...
PATCHES=(
	# glibc-2.42 added noexcept to rsqrt/rsqrtf in <math.h>; match CUDA's crt headers
	# or .cu compiles fail with "exception specification is incompatible". (2.41-r1
	# patch is for an older CUDA layout and doesn't apply to 13.0.2.)
...
src_install() {
	...
	# CUDA 13 ships real ${CUDA_PATH}/include (e.g. crt/) and possibly lib64,
	# separate from targets/.../{include,lib}. Portage's dosym refuses to
	# symlink over an existing dir, so fold any real dir into targets first,
	# then create the symlink.
	if [[ -d "${ED}/${CUDA_PATH}/include" && ! -L "${ED}/${CUDA_PATH}/include" ]]; then
		cp -a "${ED}/${CUDA_PATH}/include/." "${ED}/${CUDA_PATH}/targets/${narch}-linux/include/" || die "fold include"
		rm -rf "${ED}/${CUDA_PATH}/include" || die "rm real include"
	fi
	dosym "targets/${narch}-linux/include" "${CUDA_PATH}/include"
	if [[ -d "${ED}/${CUDA_PATH}/lib64" && ! -L "${ED}/${CUDA_PATH}/lib64" ]]; then
		cp -a "${ED}/${CUDA_PATH}/lib64/." "${ED}/${CUDA_PATH}/targets/${narch}-linux/lib/" || die "fold lib64"
		rm -rf "${ED}/${CUDA_PATH}/lib64" || die "rm real lib64"
	fi
